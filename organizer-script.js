...
if (!document.getElementById('frame')) {
  ...
  // UI wiring
  hambtn.addEventListener('click', ()=> profilePanel.classList.toggle('hidden'));
  ...
  // panel buttons
  panelRecentBtn.addEventListener('click', ()=> renderPanel('recent'));
  panelAddBtn.addEventListener('click', ()=> renderPanel('add'));
  panelManageBtn.addEventListener('click', ()=> renderPanel('manage'));
  ...
  // load organizer events (client-side sort to avoid composite index requirement)
  async function loadOrganizerEvents(uid) {
    try {
      ...
      const events = snap.docs
        ...
        .sort((a, b) => {
          const ta = a.createdAt ? (a.createdAt.seconds ?? (a.createdAt.toMillis ? a.createdAt.toMillis() : 0)) : 0;
          const tb = b.createdAt ? (b.createdAt.seconds ?? (b.createdAt.toMillis ? b.createdAt.toMillis() : 0)) : 0;
          return tb - ta;
        });
      return events;
    } catch (err) {
  ...
  async function renderPanel(view) {
    ...
    if (view === 'recent') {
      ...
      frame.innerHTML = '';
      if (events.length === 0) {
        frame.appendChild(el('div',{text:'No upcoming events.', class:'placeholder'}));
        return;
      }
      events.forEach(ev => {
        ...
        const right = el('div');
        const details = el('div',{text: `Organizer: ${ev.organizerName || '—'}`, class:'meta'});
        const regBtn = el('button',{text:'Register (test)', class:'btn'});
        regBtn.addEventListener('click', async ()=> {
          try {
            await addDoc(collection(db,'registrations'), {
              eventId: ev.id,
              studentUid: currentUser.uid,
              // note: this test registration won't include studentName/phone snapshot
              createdAt: serverTimestamp(),
              status: ev.autoApprove ? 'approved' : 'pending'
            });
            showToast('Registered (test)');
          } catch(err) {
            console.error(err); showToast('Register failed');
          }
        });
    ...
    if (view === 'add') {
      ...
      form.appendChild(el('label',{html:`Title<br><input id="evtTitle" class="txt" />`}));
      form.appendChild(el('label',{html:`Date<br><input id="evtDate" type="date" class="txt" />`}));
      form.appendChild(el('label',{html:`Time<br><input id="evtTime" class="txt" placeholder="08:00 AM" />`}));
      form.appendChild(el('label',{html:`Location<br><input id="evtLocation" class="txt" />`}));
      form.appendChild(el('label',{html:`No. of Volunteers<br><input id="evtLimit" type="number" class="txt" min="1" />`}));
      form.appendChild(el('label',{html:`Description<br><textarea id="evtDesc" class="txt" rows="4"></textarea>`}));
      form.appendChild(el('label',{html:`Auto-approve registrations<br><input id="evtAuto" type="checkbox" />`}));
      const createBtn = el('button',{text:'Create Event', class:'btn'});
      ...
      const cancelBtn = el('button',{text:'Cancel', class:'btn',});
      cancelBtn.style.background = '#ccc'; cancelBtn.style.color = '#000';
    ...
    if (view === 'manage') {
      ...
      frame.innerHTML = '';
      if (events.length === 0) {
        frame.appendChild(el('div',{text:'You have not created any events yet.', class:'placeholder'}));
        return;
      }
      events.forEach(ev => {
        const box = el('div',{class:'event'});
        ...
        left.appendChild(el('div',{text: `${ev.date || ''} • ${ev.time || ''} • ${ev.location || ''}`, class:'meta'}));
        const right = el('div');
        const manageBtn = el('button',{text:'Open', class:'btn'});
  ...
  // detailed event manage view: list registered students, send message
  async function openEventManageView(ev) {
    frame.innerHTML = '';
    const header = el('div',{class:'card'});
    header.appendChild(el('h3',{text:`${ev.title}`}));
    header.appendChild(el('div',{text:`${ev.date || ''} • ${ev.time || ''} • ${ev.location || ''}`, class:'muted'}));
    header.appendChild(el('div',{text: ev.description || '', class:'muted'}));
    ...
    if (regs.length === 0) {
      listWrap.appendChild(el('div',{text:'No registrations yet', class:'placeholder'}));
      frame.appendChild(listWrap);
    } else {
      // render each registration; prefer snapshot fields in registration
      for (const r of regs) {
        let displayName = r.studentName || '';
        let phone = r.studentPhone || '';

        // fallback: try reading users/uid if needed (wrapped)
        ...
        const left = el('div');
        left.appendChild(el('div',{text: displayName, style:'font-weight:600'}));
        left.appendChild(el('div',{text: phone, class:'meta'}));
        const right = el('div');
        const initials = (displayName || 'ST').split(' ').map(s=>s[0]).slice(0,2).join('').toUpperCase();
        right.appendChild(el('div',{text: initials, class:'avatar'}));
    ...
    msgBox.appendChild(el('textarea',{id:'evtMsgInput', rows:4, class:'txt'}));
    const sendBtn = el('button',{text:'Send to participants', class:'btn'});
    sendBtn.addEventListener('click', async ()=> {
      const text = document.getElementById('evtMsgInput').value.trim();
      if (!text) { showToast('Enter a message'); return; }
      await sendMessageToParticipants(ev.id, text);
      document.getElementById('evtMsgInput').value = '';
    });
  ...
  onAuthStateChanged(auth, async (user) => {
    ...
    try {
      ...
      profileName.textContent = data.name || 'Organizer';
      profileRole.textContent = data.role || 'Organizer';
      profileInitial.textContent = (data.name || 'OR').split(' ').map(s=>s[0]).slice(0,2).join('').toUpperCase();
      profileQuick.textContent = `UID: ${user.uid}`;

      // default view
      renderPanel('manage');

      // notification count demo: pending approvals for this organizer
      try {
        const q = query(collection(db,'registrations'), where('status','==','pending'));
        const snap = await getDocs(q);
        const pending = snap.size;
        if (pending > 0) { notifCount.textContent = pending; notifCount.classList.remove('hidden'); }
...
// Adds a ripple effect to buttons (works on dynamic elements too via delegation)

(function attachRipples() {
  // Use delegation on document — faster and works for buttons added later.
  document.addEventListener('pointerdown', function (ev) {
    // Only act for primary button events
    if (ev.button !== 0) return;

    // Acceptable selectors for buttons we want to decorate:
    const selector = '.organizer-page .btn, .organizer-page .action-btn, .organizer-page .tab-btn, .organizer-page .link-btn, .organizer-page .icon-btn, .btn, .action-btn, .tab-btn, .link-btn, .icon-btn';
    ...
    // size: take larger of width/height
    const size = Math.max(rect.width, rect.height) * 0.9;
    ripple.style.width = ripple.style.height = size + 'px';

    // position: center ripple at pointer location relative to button
